...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="initialColours" as="xs:string" select="$xslwebapp || '/xml/black-tangram.xml'"/>
    ...
    <xsl:param name="xslwebapp" as="xs:string" select="'/declarative-amsterdam'"></xsl:param>
        ...
        <xsl:choose>
            <xsl:when test="$basic-shape-map($oldShape) eq $basic-shape-map($newShape)">
                ...
                <xsl:call-template name="next-tangram">
                    ...
                    <xsl:with-param name="settings" as="document-node()" select="doc($xslwebapp || '/xml/tangram/' || $newShape || '.xml')"/>
                </xsl:call-template>
            </xsl:when>
            <xsl:otherwise>
                <xsl:call-template name="next-tangram">
    ...
    <xsl:template name="do-moves">
        ...
        <xsl:variable name="transform" as="xs:string" select="
            ...
            "/>
        <ixsl:set-attribute name="transform" select="$transform" object="$svgobject"/>
