<xsl:transform
  ...
  <xsl:template name="move-rect">
    ...
    <!-- Define a variable $new-x based on the current @x attribute of the $rect rectangle. -->
    ...
    <xsl:if test="$go-on">
      <!-- Change the @x attribute of the $rect rectangle to $new-x -->
      
      <!-- YOUR CODE HERE -->
      <ixsl:set-attribute name="x" select="string($new-x)" object="$rect"/>
      
      <!-- Schedule a recursive call of the move-rect template; this will move the rectangle furter. -->
